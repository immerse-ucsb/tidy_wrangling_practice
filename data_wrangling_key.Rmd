...
summary(penguins)
glimpse(penguins)
head(penguins) # Use head() to check if we need to "tidy" the data

#We do not need to "tidy" data as the `penguins` dataset is already in tidy format
```
...
peng_subset <- penguins %>%
  select(species, bill_depth_mm, body_mass_g) %>% # Select three variables
  filter(species %in% c('Adelie', 'Gentoo')) %>% # Filter by two species
  mutate(body_mass_g = ifelse(is.na(body_mass_g), -999, body_mass_g), # One way to relabel NA
         bill_depth_mm = replace_na(bill_depth_mm, -1)) # Better way to relabel NA
```
...
peng_sum <- penguins %>%
  group_by(species) %>%
  mutate(mass_normalized = body_mass_g / max(body_mass_g, na.rm = TRUE)) %>% # create new variables
...
peng_cats <- penguins %>%
  ...
  mutate(bd_cat = case_when(bd < 15.6 ~ 'shallow',
                            bd > 18.7 ~ 'deep',
                            TRUE ~ 'medium')) %>% #anything in between
